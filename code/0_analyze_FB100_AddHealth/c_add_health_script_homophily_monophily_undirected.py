...
if __name__=="__main__":
    ...
    for f in listdir(args.input_dir):
        if f.endswith(args.file_ext): #and f=='comm22.gml': #and f!='comm25.gml': #f=='comm10.gml':
            ...
            (ah_gender_undirected, adj_directed_gender_undirected) = create_adj_membership(ah_graph,
            ...
            avg_deg_gender = []
            max_deg = []
            class_labels = np.sort(np.unique(np.array(ah_gender_undirected)))
            for i in range(len(class_labels)):
                block_size_gender.append( np.sum((ah_gender_undirected==class_labels[i])+0))
                proportion_gender.append( np.mean(ah_gender_undirected==class_labels[i]))
                avg_deg_gender.append(np.mean(np.array(np.sum(adj_directed_gender_undirected,1))[ah_gender_undirected==class_labels[i]]))
                max_deg.append(np.max(np.array(np.sum(adj_directed_gender_undirected,1))[ah_gender_undirected==class_labels[i]]))
            ...
            if len(block_size_gender) >= 2:
                ## AH - homophily
                homophily_gender =  homophily_index_Jackson_alternative(adj_directed_gender_undirected, ah_gender_undirected) # observed homophily
                ...
                b0_temp = np.exp(monophily_index_overdispersion_Williams_with_intercept(np.matrix(adj_directed_gender_undirected), np.array(ah_gender_undirected)))
                ...
                ## AH - monophily
                monophily_gender = monophily_index_overdispersion_Williams(adj_directed_gender_undirected, ah_gender_undirected)
                ...
                chi_square_p_value_gender = compute_chi_square_statistic(np.matrix(adj_directed_gender_undirected), np.array(ah_gender_undirected))
